...
function storeIncident(incident) {
  try {
    const stmt = db.prepare(`
        ...
        (id, container_name, fingerprint, incident_summary, resolution, action_taken, outcome, mttr_seconds, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, unixepoch())
    `);
    ...
    // Maintain max 1000 incidents (LRU eviction based on created_at)
    // Actually, simple DELETE of oldest is easier
    const count = db.prepare('SELECT COUNT(*) as count FROM incident_memory').get().count;
    if (count > 1000) {
        db.prepare('DELETE FROM incident_memory WHERE id IN (SELECT id FROM incident_memory ORDER BY created_at ASC LIMIT 1)').run();
    }
  } catch (e) {
